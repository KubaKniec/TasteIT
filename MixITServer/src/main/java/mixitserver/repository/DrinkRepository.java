...
//    List<Drink> findByNameContainingOrderByPopularityDesc(String drinkName);
    ...
    @Query("SELECT drink FROM Drink drink WHERE " +
            ...
            "  NOT EXISTS (" +
            "    SELECT i FROM Ingredient i JOIN i.drinks d WHERE d = drink AND i.name NOT IN :ingredientNames" +
            "  ) " +
            "  OR :matchType = 'AT_LEAST' AND " +
            "  (SELECT COUNT(DISTINCT i.name) FROM Ingredient i JOIN i.drinks d WHERE d = drink AND i.name IN :ingredientNames) >= :minIngredientCount " +
